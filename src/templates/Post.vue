  ...
  <Layout>
    <post-header :post="$page.post" />
      ...
      <article class="max-w-xl md:max-w-2xl xl:max-w-3xl mx-auto px-6 sm:px-12 pt-16" :class="{'border-b border-grey-lighter pb-10 mb-16': !$page.post.author}">
        ...
        <div :class="{'pb-10': $page.post.author || $page.post.tags}" class="prose prose-lg markdown text-lg leading-normal text-gray-700" v-html="$page.post.content" />

        <footer v-if="$page.post.author || $page.post.tags" class="flex flex-wrap pb-10 sm:pb-16">
          <div>
            <g-link v-for="tag in $page.post.tags" :key="tag.id" :to="`${tag.path}/`" class="inline-block text-indigo-400 hover:text-white hover:bg-indigo-400 border border-indigo-400 font-sans font-bold text-xs sm:text-sm px-4 py-2 mr-4 mb-2 rounded-full transition-color transition-bg">
          ...
          <div v-if="$page.post.author" class="flex flex-wrap items-center justify-center sm:justify-left border-t border-b border-gray-300 w-full mt-10 py-10 sm:px-16">
            ...
            <div class="px-4 sm:w-4/5 text-center sm:text-left">
              <h4 class="font-sans font-bold text-lg sm:text-xl mb-2 sm:mb-4">
                <span class="text-black hover:text-gray-600 capitalize border-b-2 border-transparent transition-color">JustSteveKing</span>
              </h4>
            </div>
          </div>
        </footer>
...
export default {
  ...
  computed: {
    ...
    ogImageUrl () {
      return this.$page.post.cover || `${this.config.siteUrl}/images/juststeveking-card.png`
    }
  },
  components: {
    Alert,
